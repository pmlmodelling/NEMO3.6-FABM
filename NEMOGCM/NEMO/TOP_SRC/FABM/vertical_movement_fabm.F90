...
#  include "domzgr_substitute.h90"
   ...
   PUBLIC compute_vertical_movement_1, compute_vertical_movement_3
   ...
   SUBROUTINE compute_vertical_movement_1( kt )
      !!----------------------------------------------------------------------
      !!                     ***  compute_vertical_movement_1  ***
      !!
      !! ** Purpose : compute vertical movement of FABM tracers through the water
      !!              (sinking/floating/active movement)
      !!
      !! ** Method  : Retrieves additional vertical velocity field and performs
      !!              simple 1st order upwind advection.
      !!----------------------------------------------------------------------
      !
         ...
         DO ji=1,jpi ! i-loop
            ...
            IF (k_floor > 1) THEN
               ...
               DO jn=1,jp_fabm ! State loop
                  ...
                  flux(0) = 0._wp
                  DO jk=1,k_floor-1 ! k-loop
                     IF (w_if(jk) > 0) THEN
                        ! Upward movement (source layer is jk+1)
                        flux(jk) = w_if(jk) * trn(ji,jj,jk+1,jp_fabm_m1+jn)
                     ELSE
                        ! Downward movement (source layer is jk)
                        flux(jk) = w_if(jk) * trn(ji,jj,jk,jp_fabm_m1+jn)
                     END IF
                  END DO
                  flux(k_floor:) = 0._wp

                  ! Compute change (per volume) due to vertical movement per layer
                  dc(1:k_floor) = (flux(1:k_floor) - flux(0:k_floor-1)) / fse3t(ji,jj,1:k_floor)
                  tra(ji,jj,1:k_floor,jp_fabm_m1+jn) = tra(ji,jj,1:k_floor,jp_fabm_m1+jn) + dc(1:k_floor)

#if defined key_trdtrc && defined key_iomput
                  ! Store change due to vertical movmeent as diagnostic
   ...
   END SUBROUTINE compute_vertical_movement_1
   
   SUBROUTINE compute_vertical_movement_3( kt )
      !!----------------------------------------------------------------------
   ...
   END SUBROUTINE compute_vertical_movement_3
