MODULE diainsitutem
   ...
   !!======================================================================
   !! Initial version: D. Acreman June 2006 Original Code
   !! History :  3.6  !  2014  (E O'Dea) for 3.6 25hour mean output
   !!----------------------------------------------------------------------
  ...
  REAL(wp), PUBLIC, ALLOCATABLE, DIMENSION(:,:,:) :: rinsitu_t
...
CONTAINS
   ...
   REAL FUNCTION insitu_tem_alloc()
      ...
      ierr = 0
      !
      ALLOCATE( rinsitu_t(jpi,jpj,jpk), STAT=ierr(1) )
         !
...
!-----------------------------------------------------------------------------------
  ...
  SUBROUTINE theta2t()
    ...
    INTEGER, PARAMETER :: inum_steps=10                ! number of steps in integration
    INTEGER            :: jstep                        ! iteration counter
     ...
     DO jk = 1, jpkm1
        DO jj = 1, jpj
           DO ji = 1, jpi
              ...
              zDP(ji,jj,jk) = fsdept(ji,jj,jk) / real(inum_steps)
           END DO
        END DO
     END DO
...
! zTB is the temperature at pressure zP-zdP and zTA is the temperature at pressure zP+zdP
     ...
     interation: DO jstep=1, inum_steps
